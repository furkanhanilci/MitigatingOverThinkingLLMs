...
class vLLMRollout(BaseRollout):
    ...
    def generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        if "max_tokens" in kwargs:
            response_max_len = kwargs["max_tokens"]
        else:
            response_max_len = self.config.response_length

        if response.shape[1] < response_max_len:
            response = pad_sequence_to_length(
                response, response_max_len, self.pad_token_id
            )
            log_probs = pad_sequence_to_length(
                log_probs, response_max_len, self.pad_token_id
            )
