...
def compute_advantage(
    ...
    if adv_estimator == "gae":
        ...
        if add_process_reward:
            process_advantages, process_returns = (
                core_algos.compute_grpo_process_advantage(
...
def compute_data_metrics(batch, use_critic=True):
    ...
    metrics = {
        ...
        **(
        ...
        # level score & length
        **{
            f"level_score/{level}": torch.mean(level_score[level]).detach().item()
            for level in level_score.keys()
        },
        **{
            f"level_length/{level}": torch.mean(level_length[level]).detach().item()
            for level in level_length.keys()
        },
    }
    return metrics
...
class RayPPOTrainer(object):
    ...
    async def async_validate(self):
        ...
        for test_data in self.val_dataloader:
            ...
            val_result.non_tensor_batch["data_source"] = (
            ...
            if todo_key in all_val_result.non_tensor_batch:
                ...
                for sample in per_sample_results:
                    ...
                    for passk in [1, 2, 4, 8]:
                        if passk <= len(sample):
                            tmpscore = [
                                max(scores[i : i + passk])
                                for i in range(0, len(scores), passk)
                            ]
    ...
    async def fit_async(self):
        # This is mostly the fit function, we copy it for now
        """
            ...
            for batch_dict in self.train_dataloader:
                ...
                with _timer("step", timing_raw):
                    ...
                    with _timer("async_computes", timing_raw):
                        # Schedule the blocking calls concurrently.
                        tasks = [
                            asyncio.create_task(
                                asyncio.to_thread(
                        ...
                        # Expecting the below order:
                        # results[0] -> old_log_prob
                        # results[1] -> reward_tensor
                        # results[2] -> ref_log_prob (if use_reference_policy), and
                        # results[3] -> critic values (if use_critic)
                    ...
                    with _timer("rm_score", timing_raw):
                        ...
                        if (
                            ...
                            >= self.config.reward_model.policy_warmup_step
                    ...
                    if (self.use_critic
                        and self.global_steps
                        >= self.config.reward_model.critic_warmup_step
                    ):
